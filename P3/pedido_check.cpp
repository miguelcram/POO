/**
 * pedido_check.cpp - Comprobación estática de código fuente de las clases
 * de la P3: Articulo, Tarjeta, Usuario, Pedido, Pedido_Articulo
 * y Usuario_Pedido
...
int main(int argc, const char **argv){
  ...
  checkCode c1(argc, argv, "articulo.cpp",
	       "Orden: ./pedido_check articulo.cpp tarjeta.cpp"
	       " usuario.cpp pedido.cpp pedido-articulo.cpp"
	       " usuario-pedido.cpp -- -std=c++14 -I../P1");
  ...
  if(c1.findClass({"Articulo"})){
    ...
    c1.invocationsFromHeaders(functionNames, headerName, true,
			      ...
			      " de la biblioteca estándar como strlen,"
			      " strcpy...");
  ...
  if(c2.findClass({"Tarjeta"})){
    ...
    c2.deletedMethod({"Tarjeta", "operator="}, {{"const class Tarjeta &"},
	  {"const class Tarjeta &"}}, "Tarjeta", {"noconst", "noconst"},
    ...
    c2.inlineMethod(methodNames, parametersMethods, "Tarjeta",
		    ...
		    " con pocas instrucciones, como 'numero()',"
		    " 'caducidad()' o 'activa()'.");
    c2.check();
  }
  else
  ...
  if(c3.findClass({"Usuario"})){
    ...
    c3.deletedMethod({"Usuario", "operator="}, {{"const class Usuario &"},
	  {"const class Usuario &"}}, "Usuario", {"noconst", "noconst"},
      "Revisa el enunciado respecto a la copia de objetos.");	
    ...
    c3.inlineMethod(methodNames, parametersMethods, "Usuario",
		    {"const", "const", "const"},
		    "Sugerencia: incluir marca 'inline' a aquellos métodos"
		    " con pocas instrucciones, como "
		    "'id()', 'nombre()' o 'apellidos()'.");
    c3.guardClauses("usuario.hpp",
		    "Recuerda añadir las guardas de inclusión.");	
    c3.check();
  }
  else
  ...
  if(c4.findClass({"Pedido"})){
    ...
    c4.memberVariable("Pedido", {"tarjeta_"}, {"const"}, {true},
		      "Revisa el enunciado respecto al atributo de Tarjeta.");
  ...
  if(c5.findClass({"Pedido_Articulo", "LineaPedido"})){
    ...
    c5.numberOfConstructors("LineaPedido", 1, false,
			    "Revisa el enunciado respecto a la"
    ...
    //'pedir' sobrecargado puede hacerse mediante la búsqueda de dos
    // métodos con diferentes parametros.
  ...
  if(c6.findClass({"Usuario_Pedido"})){
    ...
    c6.invocationsFromHeaders(functionNames, headerName, true,
			      ...
			      "la biblioteca estándar como strlen, strcpy...");
