/** compra_check.cpp
 * Comprobación estática de código fuente para la P2 de POO, casos de uso
 * 1 y 2 del S. G. L.
...
int main(int argc, const char **argv){  
  ...
  c2.setIncorrectMessage("REVISA LOS POSIBLES ERRORES DE Tarjeta.");
  ...
  c3.setIncorrectMessage("REVISA LOS POSIBLES ERRORES DE Usuario.");
  if(c3.findClass({"Usuario"})){
    ...
    c3.inlineMethod(methodNames, parametersMethods, "Usuario",
		    {"const", "const", "const"},
		    "Sugerencia: incluir marca 'inline' a aquellos métodos"
		    " con pocas instrucciones, como 'id()', 'nombre()'"
		    " o 'apellidos()'.");
